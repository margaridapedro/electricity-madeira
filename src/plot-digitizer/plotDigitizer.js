...
const wpd = require("./wpd-4.3/wpd_node.js").wpd
const fs = require("fs")
...
const imagePath = getArgvValue(process.argv, "--input=");
const outputPath = getArgvValue(process.argv, "--output=");
const referenceProjectJSON = getArgvValue(process.argv, "--json-reference-file=");
const yMaxOverride = getArgvValue(process.argv, "--y-max-override=");
...
function digitizeImage(file, img) {
    ...
    if (yMaxOverride) {
        overrideYAxesMax(serializedPlotData, yMaxOverride)
    }
    ...
    fs.readdir(imagePath, function (err, files) {
        ...
        files.forEach(function (file) {
            if (path.extname(file) == fileExtension) {
                // read image
                jimp.read(path.join(imagePath, file)).then(img => {digitizeImage(file, img)});
            }
        });
    });
}
...
function overrideYAxesMax(config, value) {
    config.axesColl[0].calibrationPoints[3].dy = value
    return config
}
